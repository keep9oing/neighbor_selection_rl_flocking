"""Fine-tune from Phase 7b checkpoint (FC policy) with w_ctrl=0.2.
The policy already flocks (at FC). Fine-tuning with control cost pressure
should teach it to drop unnecessary neighbors starting from a known-good state.
Uses restore to load the checkpoint, then trains with new reward weights."""
import copy
import ray
...
if __name__ == "__main__":
    ...
    tune.run(
        GradLoggingPPO,
        name="finetune_fc_260523",
        local_dir="/workspace/test_results",
        checkpoint_freq=5,
        keep_checkpoints_num=5,
        checkpoint_at_end=True,
        stop={"training_iteration": 71},
