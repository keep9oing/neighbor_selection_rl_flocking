"""Gradual w_ctrl curriculum from FC checkpoint.
Start from checkpoint 70 (FC, trained with w_ctrl=0.02).
Fine-tune with w_ctrl=0.05 (small increase) to gently push toward selectivity.
Low lr=1e-4, entropy_coeff=0. The small w_ctrl change should keep the value
function mostly valid, avoiding the mismatch that killed the entropy_coeff fine-tune."""
import copy
import ray
...
if __name__ == "__main__":
    ...
    my_config.env.acs_train_w_ctrl = 0.05
    ...
    checkpoint_path = ("/workspace/test_results/sf_sweep_260522/"
                       "GradLoggingPPO_neighbor_selection_flocking_env_f7e41_00002_2_"
                       "scale_factor=0.1500_2026-05-22_19-39-52/checkpoint_000070")

    tune.run(
        GradLoggingPPO,
        name="curriculum_step1_260523",
