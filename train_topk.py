"""Top-K=10 neighbor selection: forces max 10 neighbors per agent.
The model's attention_scores_to_logits() masks out all but the top-K
attention scores per row. This eliminates the FC attractor by construction.
Uses original Phase 5 settings (lr=5e-4, clip=0.2, sgd_iter=10) with sf=0.15.
w_ctrl=0.02 (original value — the top-K constraint replaces w_ctrl's role)."""
import copy
import ray
