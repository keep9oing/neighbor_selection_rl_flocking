"""sf=0.15 with stabilization: w_ctrl=0.2, clip=0.1, lr=3e-4, num_sgd_iter=5.
The no-entropy trial showed sf=0.15 + w_ctrl=0.5 collapses in 2 iters.
Reducing ALL destabilizing factors: lower w_ctrl, tighter clip, lower lr,
fewer SGD steps. Goal: same learning as Phase 7b but without collapse."""
import copy
import ray
...
if __name__ == "__main__":
    ...
    # 2 trials: w_ctrl=0.2 and w_ctrl=0.1, both with sf=0.15 + stabilization
    tune.run(
        GradLoggingPPO,
        name="sf15_stable_260523",
            ...
            "env_config": {
                ...
                "config": {
                    **env_config_dict,
                    "env": {
                        **env_config_dict["env"],
                        "acs_train_w_ctrl": tune.grid_search([0.1, 0.2]),
                    },
                },
            },
            "framework": "torch",
