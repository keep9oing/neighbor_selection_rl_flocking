"""sf=0.05 with w_ctrl=0.2: slow stable learning + selectivity incentive.
Phase 5 proved sf=0.05 is stable (no collapse after 100 iters, entropy delta=-44).
That experiment converged to FC because w_ctrl=0.02 didn't reward selectivity.
With w_ctrl=0.2, the control cost signal should steer the policy toward non-FC.
Uses the ORIGINAL aggressive settings from Phase 5 (lr=5e-4, clip=0.2, sgd_iter=10)
which are known to be stable at sf=0.05."""
import copy
import ray
