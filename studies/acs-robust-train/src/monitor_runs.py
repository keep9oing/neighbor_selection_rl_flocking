"""Training monitor for acs-robust-train (both variants).

Polls the two runs' result.json every ~120 s; appends one compact line per
NEW training iteration to logs/monitor.log. EXITS (0) as soon as EITHER
train_robust.py process is gone (completion or crash) so the session gets a
notification and can react; exits (1) immediately on an error signature in a
driver log. Start AFTER both trainings are launched:
    python monitor_runs.py <pid_legacy> <pid_r0log>
"""
import glob
import json
import os
...
RUNS = {
    "R1": "/workspace/test_results/c2R1_lmix_legacy_260807",
    "R2": "/workspace/test_results/c2R2_lmix_r0log_260807",
}
LOG = "/workspace/studies/acs-robust-train/logs/monitor.log"
DRIVER_LOGS = {
    "R1": "/workspace/studies/acs-robust-train/logs/train_r1.log",
    "R2": "/workspace/studies/acs-robust-train/logs/train_r2.log",
}
...
def last_metrics(run_dir):
    files = glob.glob(os.path.join(run_dir, "GradLogging*", "result.json"))
    if not files:
        return None
    last = None
...
def main():
    pids = {"R1": int(sys.argv[1]), "R2": int(sys.argv[2])}
    seen = {"R1": -1, "R2": -1}
