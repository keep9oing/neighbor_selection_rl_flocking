...
## Phase 0 — Ground yourself + regression baseline  [ ]

- [ ] Read PROBLEM.md here, then `studies/acs-conv-knn/RUNLOG.md` (at least the
...
- [ ] Verify the plain-binary logits path in `attention_scores_to_logits`
...
- [ ] Run `python test_baselines.py` and record it green (pre-change baseline).
- [ ] `nvidia-smi` on cuda:1 and cuda:3 (both must be free-ish; they are
      user-assigned to this study).

## Phase 1 — Env: C2 termination + shaped reward + global stats  [ ]
...
- [ ] Defaults regression: default-config env == old behavior (diff a 100-step
...
- [ ] **Online/offline C2 cross-check**: run 8 episodes of the 'nearest' k=10
...
- [ ] Reward magnitude print on one scripted episode (transient vs settled):
      per-term values match the calibration table above (~-1.0 / ~-0.09 early;
      ~0 settled except ctrl).

## Phase 2 — Model: two selection heads, unsaturated; aux rework  [ ]
...
- [ ] Unit forward: both heads produce (B, 2*N*N) logits; masked/diag
...
- [ ] 2-iteration smoke train per variant (local_mode or 1 worker, CPU ok):
...
## Phase 4 — Evaluation vs the frontier  [ ]

- [ ] Checkpoint selection BY EVAL METRIC (success rate, then J), not training
      return: for each variant take best-eval checkpoint + final.
- [ ] Rollout protocol (reuse predecessor harness `common.py::rollout` +
      ...
      success, t_conv, J per episode.
- [ ] Compare against frontier references (same seeds, already computed in
...
- [ ] Generalization probes: L=125 and L=500 (frontier exists in summary_main),
      N=10/40 optional (needs matching-density L per predecessor convention).
- [ ] Adaptivity forensics (the actual research question):
      - rank-deviation: fraction of selected edges NOT in the nearest-deg_i
        set, vs time (0 == kNN mimic; expect >0 in merge phase for B);
      - degree-vs-time profiles per agent (adaptive K visible?);
      - case studies on seeds where frozen kNN10 fragments but the policy
        succeeds (the 9% — do we rescue them?).
- [ ] Report to user in Korean (numbers-first, honest re: significance).

## Phase 5 — Wrap-up  [ ]

- [ ] RUNLOG complete; REPORT (Korean) in this study dir; predecessor study's
      REPORT_KO gets a one-line pointer here.
- [ ] Decide with user: commit (explicit approval only, no AI attribution,
      never push).
